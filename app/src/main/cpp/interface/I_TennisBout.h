...
#if defined ( __TENNIS_BOUT_TEST__ ) && !defined ( __ANDROID__ ) && !defined ( __TENNIS_CONTROL_MODULE__ )
enum BoutResult {
    BR_UNKNOW,
    BR_INVALID,
    BR_FIRSTSERVERSUCCESSED,
    BR_FIRSTSERVERFAILED,
    BR_AWIN,
    BR_BWIN,
    BR_TWOHIT,
    BR_TWOSERVERFAILED,
...
#if defined ( __TENNIS_BOUT_TEST__ ) && !defined ( __ANDROID__ ) && !defined ( __TENNIS_CONTROL_MODULE__ )
    virtual void setResult( BoutResult result ) = 0;

    virtual BoutResult getResult() const = 0;
#endif
};
...
typedef I_TennisBout *( *CREATE_TENNISBOUT_INTERFACE )();
